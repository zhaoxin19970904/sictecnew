...
<TABLE cellSpacing=0 cellPadding=0 width=761 border=0>
  ...
  <TR>
    ...
    background=img/cla_bg2.gif>
            ...
            <tr>
              ...
              <table height="28" cellSpacing="0" cellPadding="0" border="0">
                <tr>
                  ...
                  <td><strong><%=classname%>访问排行榜</strong></td>
                </tr>
              </table>
              </td>
            </tr>
      ...
      <table width="590" border="0" cellspacing="0" cellpadding="0" style="border-collapse: collapse" bordercolor="#111111">
        <%
              sql="select top 5 * from userjoinclassinfo where classid='"&curclassid&"' and userstatus='成员' order by logintimes desc"
              rs.open SQL,schooldb
              while not rs.eof
            %>
        <tr valign="bottom">
          <td height="18" width="160" class="topic"><a href="javascript:personalinfo('<%=trim(rs("userid"))%>');"><%=rs("userid")%></a></td>
          <td width="100" align="left" class="topic"><%=rs("logintimes")%></td>
          <td width="270" class="digi"><%=rs("lastlogintime")%></td>
        </tr> <%
            rs.movenext
            wend
            rs.close
