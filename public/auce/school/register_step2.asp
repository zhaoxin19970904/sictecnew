...
if userid="" then
   response.redirect "error.asp?info=对不起，您注册用户的步骤不正确，请重新注册！"
end if
...
if not rs.eof then
  response.redirect "error.asp?info=对不起，您所添入的用户名已经存在！"
end if
rs.close

set rs = createobject("ADODB.recordset")
SQL = "select * from UserInfo"
rs.Open SQL,schooldb,3,2
rs.AddNew
  rs("regdate")=now()
  rs("userid")=userid
...
SQL = "select * from UsercommunicationInfo"
