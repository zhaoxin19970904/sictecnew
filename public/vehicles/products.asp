...
<%
  ...
  find_id=request("id")
  sql="select type_code from type where id="&find_id&""
  set rs=conn.execute(sql)
    find_type_code=rs("type_code")
  rs.close
%>
...
<table width="721" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      <td height="355" valign="top"> 
                        <%
		sql="select * from products where type='"&find_type_code&"' order by up_date desc"
		rs.open sql,conn,1,1
		pv_reds=rs.recordcount
if pv_reds=0 then
        rs.close
        response.write "<div align=center><font size='2'>Sorry,there is no product of this type for the moment.</font></div>"
		'response.End()
...
%>
                                  <font class="text"><a href="products.asp?page=1&id=<%=find_id%>">First 
                                  </a></font> <font class="text"><a href="products.asp?page=<%=iPage-1%>&id=<%=find_id%>">Back</a></font> 
                                  <%end if%>
                                  <%
...
%>
                                  ...
                                  Next Last</font> 
                                  <%else%>
                                  <a href="products.asp?page=<%=iPage+1%>&id=<%=find_id%>"> Next</a><a href="products.asp?page=<%=pages%>&id=<%=find_id%>"> 
                                  Last</a> 
                                  <%
end if
%>&nbsp;&nbsp;&nbsp;
